...
import inventory, index, doc, indexer
...
class Web(web.RequestHandler):
    ...
    def get(self):
        ...
        if q is None:
            return
        title_summary = pickle.load(open(inventory.GEN_STORE, 'rb'))
        ...
        for response in responses:
            for result in json.loads(response.body.decode())['results']:
                title = result['title']
                result['summary'] = title_summary[title]
...
def start_servers():
    ...
    if task_id == 0:
        app = httpserver.HTTPServer(tornado.web.Application([
                (r'/search', Web),
                (r'/(.*)', tornado.web.StaticFileHandler, {"path": inventory.WEBAPP_PATH, "default_filename": "index.html"})
            ], **SETTINGS))
        log.info('Front end is listening on %d', port)
    else:
        log_idf = pickle.load(open(inventory.DF_STORE, 'rb'))
 #       for term in log_idf:
 #           log.info('idf term' + term + " idf " + log_idf[term])
